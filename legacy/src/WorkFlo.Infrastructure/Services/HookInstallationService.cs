...
public class HookInstallationService : IHookInstallationService
{
    ...
    public async Task<Result> InstallHooksAsync(bool force = false)
    {
        ...
        // Install hooks
        string[] hooks = new[] { "pre-commit", "commit-msg" };

        foreach (string? hookName in hooks)
        {
            string hookPath = Path.Combine(GitHooksDirectory, hookName);
            ...
            string hookContent = GetHookTemplate(hookName);
    ...
    public async Task<Result<bool>> AreHooksInstalledAsync()
    {
        ...
        foreach (string? hookName in hooks)
        {
            ...
            string content = await File.ReadAllTextAsync(hookPath).ConfigureAwait(false);
