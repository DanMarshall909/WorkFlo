...
exports.deactivate = exports.activate = exports.WorkFloStatusProvider = void 0;
...
class WorkFloStatusProvider {
    ...
    async fetchGitHubIssue(issueNumber, workspacePath) {
        try {
            console.log(`Fetching GitHub issue #${issueNumber}`);
            // Add timeout to prevent hanging
            const timeout = 10000; // 10 seconds
            (0, child_process_1.exec)(`gh issue view ${issueNumber} --json title,url,body,state,assignees,labels,createdAt,updatedAt`, {
                cwd: workspacePath,
                timeout: timeout
            }, (error, stdout, stderr) => {
                if (error) {
    ...
    async fetchOpenIssues(workspacePath) {
        try {
            console.log('Fetching open GitHub issues');
            const timeout = 10000; // 10 seconds
            (0, child_process_1.exec)(`gh issue list --state open --limit 10 --json number,title,url,labels,state`, {
                cwd: workspacePath,
                timeout: timeout
            }, (error, stdout, stderr) => {
                if (error) {
    ...
    async startTDDSession(issueNumber) {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
        ...
        try {
            ...
            (0, child_process_1.exec)(`${workFloPath} start ${issueNumber}`, { cwd: workspaceFolder.uri.fsPath }, (error, stdout, stderr) => {
                if (error) {
    ...
    async createNewIssue() {
        ...
        try {
            (0, child_process_1.exec)('gh issue create --web', { cwd: workspaceFolder.uri.fsPath }, (error, stdout, stderr) => {
                if (error) {
    ...
    <body>
        ...
        ` : `
            ...
            issue.labels.map(label => `<span style="
                                                    background-color: var(--vscode-badge-background);
                                                    color: var(--vscode-badge-foreground);
                                                    padding: 2px 6px;
                                                    border-radius: 10px;
                                                    font-size: 10px;
                                                ">${label.name}</span>`).join(' ') : ''}
                                    </div>
                                    <div style="
                                        font-size: 13px;
                                        line-height: 1.4;
...
exports.WorkFloStatusProvider = WorkFloStatusProvider;
WorkFloStatusProvider.viewType = 'workfloStatus';
function activate(context) {
    const provider = new WorkFloStatusProvider(context.extensionUri);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(WorkFloStatusProvider.viewType, provider));
    ...
    provider.onStatusChanged = (status) => {
        if (status.active) {
            const phaseIcon = { start: '🟡', red: '🔴', green: '🟢', refactor: '🔵', cover: '🟣', next: '🟡' };
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('workflo.showStatus', () => {
