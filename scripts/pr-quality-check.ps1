...
<#
.SYNOPSIS
    Comprehensive PR Quality Check Script for Anchor Project
.DESCRIPTION
...
Write-Header "🚀 Starting PR Quality Check for Anchor Project"
...
try {
    ...
    if (Test-Command "jb") {
        try {
            $resharperCmd = "jb inspectcode Anchor.sln --output=`"$resharperOutputFile`" --format=Xml --severity=WARNING"
